...
> Additionally, you can "automate" the value by setting this: `-Xms$(({{SERVER_MEMORY}} - ({{SERVER_MEMORY}}/**20**)))M -Xmx$(({{SERVER_MEMORY}} - ({{SERVER_MEMORY}}/**20**)))M'`
> As you can see, the  `**20**` I've set above indicates how much RAM after divided to 20, subsequently the Server's allocated RAM would minus that divided value and SHOULD be able to run well   
___
